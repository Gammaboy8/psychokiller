const sharp = require('C:/Users/ayush/AppData/Roaming/Genspark Claw/bundled-resources/openclaw/node_modules/sharp/lib/index.js');
...
async function main() {
  ...
  const logoRound = await sharp(SRC)
    .resize(340, 340)
    .composite([{ input: Buffer.from(`<svg width="340" height="340"><circle cx="170" cy="170" r="170" fill="#fff"/></svg>`), blend: 'dest-in' }])
    .png().toBuffer();

  await sharp(bg).composite([{ input: logoRound, left: 65, top: 145 }]).png().toFile(path.join(PUB, 'og.png'));
  log('og.png (1200x630)');
  console.log('DONE');
}
